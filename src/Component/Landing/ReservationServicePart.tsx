...
function ReservationServicePart() {
  ...
  useEffect(() => {
    ...
    handleResize(); // برای تنظیم وضعیت اولیه
    window.addEventListener("resize", handleResize); // تنظیم شنونده برای تغییر اندازه

    return () => {
      window.removeEventListener("resize", handleResize); // پاک کردن شنونده
    };
  }, []);
